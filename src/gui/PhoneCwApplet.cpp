...
void PhoneCwApplet::buildCwPanel()
{
    ...
    // ── Local Sidetone (client-side, ms-latency) ───────────────────────────
    // Generated by AudioEngine's CwSidetoneGenerator independently of the
    // radio's DAX-fed sidetone.  Useful at higher WPM where round-trip
    // latency makes the radio's sidetone unusable.
    {
        ...
        m_localSidetoneBtn = new QPushButton("Local STn");
        m_localSidetoneBtn->setCheckable(true);
        m_localSidetoneBtn->setFixedHeight(22);
        m_localSidetoneBtn->setFixedWidth(kLeftColW);
        m_localSidetoneBtn->setToolTip(
            "Local CW sidetone — generated by AetherSDR with ~10 ms latency,\n"
            "independent of the radio's DAX-fed sidetone.");
        m_localSidetoneBtn->setAccessibleName("Local CW sidetone");
        m_localSidetoneBtn->setStyleSheet(QString(kButtonBase) + kGreenActive);
        m_localSidetoneBtn->setChecked(
            s.value("CwLocalSidetoneEnabled", "False").toString() == "True");
        row->addWidget(m_localSidetoneBtn);

        m_localSidetoneVolSlider = new GuardedSlider(Qt::Horizontal);
        m_localSidetoneVolSlider->setRange(0, 100);
        m_localSidetoneVolSlider->setValue(
            s.value("CwLocalSidetoneVolume", "50").toInt());
        m_localSidetoneVolSlider->setAccessibleName("Local sidetone volume");
        m_localSidetoneVolSlider->setStyleSheet(kSliderStyle);
        row->addWidget(m_localSidetoneVolSlider, 1);

        m_localSidetoneVolLabel = new QLabel(
            QString::number(m_localSidetoneVolSlider->value()));
        m_localSidetoneVolLabel->setStyleSheet(kInsetValueStyle);
        m_localSidetoneVolLabel->setFixedWidth(kValueW);
        m_localSidetoneVolLabel->setAlignment(Qt::AlignCenter);
        row->addWidget(m_localSidetoneVolLabel);

        connect(m_localSidetoneBtn, &QPushButton::toggled, this, [this](bool on) {
            auto& ss = AppSettings::instance();
            ss.setValue("CwLocalSidetoneEnabled", on ? "True" : "False");
            ss.save();
            emit localSidetoneEnabledChanged(on);
        });
        connect(m_localSidetoneVolSlider, &QSlider::valueChanged, this, [this](int v) {
            m_localSidetoneVolLabel->setText(QString::number(v));
            auto& ss = AppSettings::instance();
            ss.setValue("CwLocalSidetoneVolume", QString::number(v));
            ss.save();
            emit localSidetoneVolumeChanged(v);
        });

        vbox->addLayout(row);

        // Pitch row: follow toggle + manual override slider (disabled when follow on)
        auto* pitchRow = new QHBoxLayout;
        pitchRow->setSpacing(4);

        m_localSidetoneFollowBtn = new QPushButton("Follow");
        m_localSidetoneFollowBtn->setCheckable(true);
        m_localSidetoneFollowBtn->setFixedHeight(22);
        m_localSidetoneFollowBtn->setFixedWidth(kLeftColW);
        m_localSidetoneFollowBtn->setToolTip(
            "Pitch follows the radio's CW pitch setting.\n"
            "Off: use the manual pitch slider.");
        m_localSidetoneFollowBtn->setStyleSheet(QString(kButtonBase) + kBlueActive);
        m_localSidetoneFollowBtn->setChecked(
            s.value("CwLocalSidetonePitchFollow", "True").toString() == "True");
        pitchRow->addWidget(m_localSidetoneFollowBtn);

        m_localSidetonePitchSlider = new GuardedSlider(Qt::Horizontal);
        m_localSidetonePitchSlider->setRange(100, 2000);  // Hz
        m_localSidetonePitchSlider->setValue(
            s.value("CwLocalSidetonePitchHz", "600").toInt());
        m_localSidetonePitchSlider->setAccessibleName("Local sidetone pitch");
        m_localSidetonePitchSlider->setStyleSheet(kSliderStyle);
        m_localSidetonePitchSlider->setEnabled(!m_localSidetoneFollowBtn->isChecked());
        pitchRow->addWidget(m_localSidetonePitchSlider, 1);

        m_localSidetonePitchLabel = new QLabel(
            QString::number(m_localSidetonePitchSlider->value()));
        m_localSidetonePitchLabel->setStyleSheet(kInsetValueStyle);
        m_localSidetonePitchLabel->setFixedWidth(kValueW);
        m_localSidetonePitchLabel->setAlignment(Qt::AlignCenter);
        pitchRow->addWidget(m_localSidetonePitchLabel);

        connect(m_localSidetoneFollowBtn, &QPushButton::toggled, this, [this](bool follow) {
            auto& ss = AppSettings::instance();
            ss.setValue("CwLocalSidetonePitchFollow", follow ? "True" : "False");
            ss.save();
            m_localSidetonePitchSlider->setEnabled(!follow);
            emit localSidetonePitchFollowChanged(follow);
        });
        connect(m_localSidetonePitchSlider, &QSlider::valueChanged, this, [this](int hz) {
            m_localSidetonePitchLabel->setText(QString::number(hz));
            auto& ss = AppSettings::instance();
            ss.setValue("CwLocalSidetonePitchHz", QString::number(hz));
            ss.save();
            emit localSidetonePitchChanged(hz);
        });

        vbox->addLayout(pitchRow);
    }
